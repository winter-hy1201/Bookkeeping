...
const listPage = readFileSync(join(root, 'src/pages/me/menu-templates/list.vue'), 'utf8')
const editPage = readFileSync(join(root, 'src/pages/me/menu-templates/edit.vue'), 'utf8')
const historyPage = readFileSync(join(root, 'src/pages/me/menu-templates/history.vue'), 'utf8')
...
test('menu templates list page keeps real data chain, actions, and return snapshot', () => {
  assert.match(listPage, /listMessageTemplates/)
  assert.match(listPage, /setDefaultMessageTemplate/)
  assert.match(listPage, /deleteMessageTemplate/)
  assert.match(listPage, /usePageReturnSnapshot/)
  assert.match(listPage, /文案模板/)
  assert.match(listPage, /默认模板会直接用于每日菜单复制/)
  ...
  assert.match(listPage, /正在读取模板列表/)
  assert.match(listPage, /还没有文案模板/)

  // No hardcoded demo templates from reference image
  const demoTemplates = ['夏季清爽菜单', '周末特色菜单', '两荤两素 · 配米饭']
  for (const demo of demoTemplates) {
    assert.doesNotMatch(listPage, new RegExp(escapeRegex(demo)))
  }
})

test('menu templates list page visual contract uses warm paper semantic tokens', () => {
...
test('menu template edit page keeps pure text form, 80px label column, chips, preview, and validation', () => {
  ...
  assert.match(editPage, /validateMenuTemplate/)
  ...
  assert.match(editPage, /insertDate/)
  assert.match(editPage, /insertLunch/)
  assert.match(editPage, /insertDinner/)
  assert.match(editPage, /useDefaultBody/)
  assert.match(editPage, /社群文案预览/)
  assert.match(editPage, /使用示例菜品/)
...
test('menu template edit page visual contract uses warm paper semantic tokens', () => {
...
test('menu template history page keeps version list, notice banner, and recovery flow', () => {
  assert.match(historyPage, /getMessageTemplate/)
  assert.match(historyPage, /listTemplateVersions/)
  assert.match(historyPage, /restoreTemplateVersion/)
  assert.match(historyPage, /历史版本/)
...
test('menu template pages retain route and title semantics in pages.json', () => {
  const listEntry = pages.pages.find((p) => p.path === 'pages/me/menu-templates/list')
  const editEntry = pages.pages.find((p) => p.path === 'pages/me/menu-templates/edit')
  const historyEntry = pages.pages.find((p) => p.path === 'pages/me/menu-templates/history')

  assert.ok(listEntry)
  assert.ok(editEntry)
  assert.ok(historyEntry)
  assert.equal(listEntry.style?.navigationBarTitleText, '文案模板')
  assert.equal(editEntry.style?.navigationBarTitleText, '新建模板')
  assert.equal(historyEntry.style?.navigationBarTitleText, '模板历史')
})
