...
async function restore(version: MealCardTemplateVersion): Promise<void> {
  ...
  const ok = await confirmDialog(
    `恢复“${version.name}”？`,
    '当前月卡模板会先保存为新的历史版本，再恢复此版本的名称和正文；默认状态不变。',
  )
  if (!ok) return
  ...
  <scroll-view class="page" scroll-y>
    <view class="hero">
      ...
      <text class="hero-text"
        >每次实际修改都会保留编辑前的名称和正文，恢复也不会覆盖当前默认状态。</text
      >
    </view>

    <view v-if="loading" class="empty">月卡模板历史加载中...</view>
      ...
      <view v-for="(version, index) in versions" :key="version.id" class="version-card">
        <view class="version-header">
          <view>
            ...
            <text class="version-time">{{ dayjs(version.created_at).format('YYYY-MM-DD HH:mm:ss') }}</text>
          </view>
