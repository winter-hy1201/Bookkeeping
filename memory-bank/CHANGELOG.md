...
修复订单列表拖拽排序在数据多时与页面滚动冲突的 bug。

### 修复

- **拖拽滚动冲突**：`src/pages/order/index.vue` 的长按拖拽把手在外层 `<scroll-view scroll-y>` 内会与页面滚动同时触发，导致用户无法把订单拖到指定位置。重构拖拽事件绑定：
  - 触摸事件从整个 `order-item` 挪到 66rpx 的 `drag-handle` 上，缩小触摸捕获范围
  - 把手 `@touchstart.stop` 阻止 scroll-view 看到 touchstart，从根本上避免被卷入滚动跟踪
  - 长按 500ms 触发改为**阈值触发**（移动 ≥ 10px 才进入拖拽态），激活前不阻止任何事件，激活后 `event.preventDefault + stopPropagation` 双保险锁住滚动
  - 新增 `dragIntent` ref 记录 touchstart 时刻的 `mealType / index / orderId / startY`，跨阈值时才正式激活 `dragState` 并 clone 列表
- `DragState` / `dragOrders` / `dragState` / `dragSaving` / `dragClickBlockedUntil` 五个 ref 与 `finishDrag` / `dragItemHeightPx` 等 helper 全部保留，纯 UI 交互修复，无 DB schema 变更。

### 行为说明

- 把手上 `@touchstart.stop` 后，把手会完全锁住 scroll-view 滚动（无法用把手让页面继续滚）；用户需要触摸订单主体才能滚动。把手只有 66rpx，实际很少有人用它滚动，可接受。
- `dragClickBlockedUntil` 仍保留作为防御性兜底，新机制下 handle 的 `@click.stop` 已经能阻止 click 冒泡到 order-item，但保留它以防未来 `@click.stop` 被改掉。
...
| 限制 | 位置 / 表现 | 何时回头评估 |
|---|---|---|
| 把手不能滚动页面 | `@touchstart.stop` 后把手区域被锁住，需触摸订单主体才能滚动 | 用户反馈后评估是否放开 |
| H5 / 微信小程序端未验证 | 修复基于 Android HBuilderX 真机验证，H5 与小程序端的 scroll-view 实现可能不同 | 若发布到 H5 再真机验证 |
...
| # | 场景 | 关键断言 |
|---|---|---|
| 1 | 阈值前能滚动 | 长按把手，静止或仅抖动 ≤10px，页面正常滚动，无 toast |
| 2 | 跨阈值后页面停止滚动 | 移动 >10px 后，scroll-view 停止滚动，item 开始重排 |
| 3 | 阈值内不误触发重排 | 在 ±10px 范围内反复移动，`dragOrders` 不被改写 |
| 4 | 长按不移动 goDetail 不触发 | 把手上长按 1s 不动再松开，不进 detail 页，无状态变化 |
| 5 | 拖动后松手排序保存生效 | 拖到目标位置松手，toast "排序已保存"，退出再进顺序保持 |
| 6 | 折叠面板展开 / 折叠不影响拖拽 | 折叠态把手仍能触摸；折叠动画中触摸 handle 不应触发 drag |
| 7 | dragSaving 防重入 | 保存中再次长按把手，不进入新 drag |
| 8 | 拖出 section 边界 | 拖到午餐段底部 / 顶部继续拖，被 clamp 到边界 index 而非触发 scroll-view overscroll |
| 9 | 主路径回归 | 8.1 录单配送对账 / 8.4 取消订单 不受拖拽改动影响 |
