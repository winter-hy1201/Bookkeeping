...
- 统一业务表单标签列为 `label-width="100px"`，同步修正关联分隔线 / 错误提示缩进与页面契约测试；相关定向契约测试 33/33 通过。全量 Node 测试 118/118、ESLint、`vue-tsc`、H5 build、`git diff --check` PASS；`pnpm` 脚本因当前最小发布时间策略拒绝刚发布的 `@lucide/vue@1.38.0`，本轮未改锁文件。既有 CSS mask 基线已在 HBuilderX Android 模拟器确认，本轮新增页面图标与 TabBar 的 Android 视觉回归仍待执行，物理真机视觉与触摸回归仍 NOT_RUN。
...
订单模型升级为“一名客户同一天同餐次一张有效订单”，并支持次卡与微信 / 现金组合支付。

### 新增与行为变更

- **组合支付**：订单新增 `meal_card_quantity`；允许纯微信、纯现金、纯次卡、次卡 + 微信、次卡 + 现金，不允许微信 + 现金或三种混用。组合支付的次卡次数初始为空，由用户手动填写。
- **一餐一单**：同一 `customer_id + order_date + meal_type` 再次新增时按本次增量合入已有 pending 订单，保留原 ID 与 `sort_order`；delivered 阻止继续新增，cancelled 不复活。
- **合并保护**：支付渠道冲突时拒绝保存并引导编辑；新旧单价不同时，二次确认弹窗展示旧单价、新单价、受影响货币份数与重算前后金额。编辑改变客户 / 日期 / 餐次后撞到另一张 pending 订单时另行确认合并。
- **次卡预占**：创建 / 编辑不修改 `used_meals`，但 API 在事务内按“客户实际剩余 - 其他 pending 预占”校验。修改充值记录总次数时也不得侵占已有 pending 预占。
- **整单流转**：配送只扣 `meal_card_quantity`，不足时订单状态、排序、usage 和部分扣次全部回滚并引导去编辑支付；删除 delivered 组合订单按 `meal_card_usages` 精确回滚次卡部分。
- **列表与详情**：订单列表删除最右侧“次卡 / 价格”，副标题按“餐次 · 总份数 · 次卡次数 · 微信/现金金额 · 完整备注”换行显示；详情拆分展示次卡次数、货币份数、实际单价和货币金额。

### 修复

- 修复“客户次卡只剩 1 次，但新增 2 份纯次卡订单仍可保存”的问题。页面立即显示所需 / 可用次数，API 在事务内再次裁决，不能通过旧页面状态或重复点击绕过。
- 修复单一 SQLite 连接上并发顶层事务被误判为嵌套的问题：`tx()` 改为队列串行，订单详情配送 / 取消 / 删除同步加锁，防止双击或多页操作交错扣次。
- 纯次卡增量不再用其参考价覆盖已有货币单价；从纯次卡切换为货币 / 组合支付且客户无默认价时，必须手动填写单价。
- 修复新增页快速切换客户 / 日期 / 餐次时可能携带上一上下文单价保存的竞态；新上下文加载成功前禁止保存，失败时保持禁用并提供重试。

### Schema 与兼容

- schema 升级到 **v5**：`orders` 新增 `meal_card_quantity INTEGER NOT NULL DEFAULT 0`；v4 升级时旧纯次卡订单按 `quantity` 回填，其他旧订单为 0。
- 升级 / 恢复时仅自动合并单价与货币渠道兼容的历史 pending 重复单；冲突、delivered 和 cancelled 历史保留并给出诊断，不增加会阻断老数据升级的唯一索引。
- v5 备份随 orders 导入导出新字段；导入 v1-v4 时按旧 `payment_method` 推导，`meal_card_usages` 继续作为已配送扣次 / 删除回滚事实来源。
- 统计公式不变：`orders.amount` 只记录微信 / 现金部分，开卡收入仍通过 `meal_cards.amount` 计入，避免重复收入。

### 验证门禁

- `pnpm test`：订单规则、事务并发、列表文案与 SQLite schema / migration 共 21 条自动检查通过。
- `pnpm type-check`、`pnpm lint`、`pnpm build:h5` 通过；H5 仅有既有 uni-ui / Dart Sass deprecation warnings。
- HBuilderX 真机待回归：四种一级支付形态、组合支付手填次数、重复增量合并与改单价弹窗、预占冲突、配送 / 删除 usage、v4 → v5、v1-v5 备份往返、列表长备注和统计口径。

---

## v1.13（2026-07-22）

建立订单录入与对账的 UI 基线，不改订单、金额、SQLite 或统计公式。

### UI 与工作流

- **项目级视觉基线**：新增 `docs/design.md`，定义 `$hej-*` 语义 token、按钮 / 状态 / 空态规则和 UI 验收清单；`src/uni.scss` 提供对应全局 token。
- **今日页回滚**：按用户反馈恢复既有今日概览、状态计数和待配送 / 已配送 / 已取消三组订单布局；本版本不再改变首页工作流。
- **订单空态与录单流**：订单列表空态可创建当前所选日期的首单；新建订单自动带入该日期。新建与编辑订单统一按“订单对象 → 配送与支付 → 金额 → 备注”分区，确认区固定在底部并展示本次实际金额和支付拆分。
- **按钮对齐**：本轮自定义按钮统一使用固定高度与等值行高；主、次和危险按钮显式居中，避免 App 端文字偏移。
- **订单顶部操作区**：日期 / 新建区增加内边距，日期控件使用弱表面，新建按钮使用 `$hej-color-accent` 实心蓝；订单列表与拖拽布局不变。
- **录单引导**：合并、已配送冲突、次卡可用次数、加载 / 失败和字段占位提示改为解释当前情况与下一步。
- **金额名称**：新建 / 编辑确认区和订单详情将“货币金额”改为“实际金额”；仅改展示名称，订单金额与支付计算不变。
- **支付摘要与分区**：固定确认区将“货币支付”改为按当前渠道显示的“微信支付”或“现金支付”；配送与支付、金额合并为一张表单卡，金额保留为内部分隔小节。
- **对账统计**：指标命名改为入账收入、支出、利润、有效订单、平均每单收入；“收入趋势”改为“收支 / 利润趋势”，每个日期同时显示入账、支出和正 / 负利润。

### 修复

- **token 编译**：订单页漏写 `lang="scss"` 曾使 `$hej-*` 原样输出、App 回退默认样式；现已修复，并新增静态回归测试防止遗漏。

### 保持不变

- 一餐一单、组合支付、次卡 pending 预占、配送扣次、合并 / 单价确认与全部 SQLite 写入路径不变。
- 统计数据仍由既有 `src/api/stats.ts` 计算；本次仅改变统计页的内容结构和视觉编码。

### 验证门禁

- `pnpm test`（22 条）、`pnpm type-check`、`pnpm lint`、`pnpm build:h5` 和 `git diff --check` 通过；H5 仅输出既有 uni-ui / Dart Sass deprecation warnings。
- HBuilderX 真机待验证：今日页既有概览与三组订单、订单页顶部日期 / 新建区的间距和蓝色主按钮、订单空态日期带入、新建 / 编辑配送与支付卡内的金额小节、微信 / 现金份数摘要、固定确认区不遮挡最后字段、按钮文字垂直居中，以及统计页的正 / 负利润与长日期范围可读性。

---

## v1.14（2026-07-22）

新建订单页改为高频录单优先的单卡工作流；不改订单、金额、SQLite 或统计口径。

### UI 与工作流

- **连续录单**：顶部把日期和餐次收敛为紧凑“配送安排”，主内容改为一张连续录单卡；客户、份数、支付、金额和常显单行备注保持在一次扫读范围内。
- **支付收敛**：微信、现金、次卡保留为一级选项；组合支付只在份数大于 1 时出现。用户主动进入组合支付后，次卡次数预填 1 次，再使用步进器在合法范围内调整，补款渠道与金额自动更新。
- **金额与提示**：有货币部分时，实际单价始终直接显示输入框；选定客户后带入默认 / 已有订单单价。正常次卡仅显示可用与本次所需，有预占或不足才显示完整余额明细。常规缺失项直接反映在底部主按钮与字段附近。
- **保存结束态**：保存成功后弹出“继续下一单 / 结束录单”；继续清空客户、份数、单价和备注，保留日期、餐次与支付设定；结束或 Android 返回键返回对应日期的订单列表。

### 保持不变

- 一餐一单、pending 增量合并、delivered 阻断、次卡 pending 预占、配送扣次、支付冲突和改单价确认仍由既有 API / 事务裁决。
- 组合支付不会因次卡余额自动进入；只有用户主动选择后才预填 1 次，仍不能绕过用户选择或调整。未新增依赖、schema、迁移或 API。

### 验证门禁

- `pnpm test`（22 条）、`pnpm type-check`、`pnpm lint`、`pnpm build:h5` 和 `git diff --check` 通过；H5 构建仅输出既有 uni-ui / Dart Sass 弃用警告。
- 390 × 844 H5 浏览器检查确认配送安排条、单卡、底部确认区和组合支付展开的几何结构；H5 本身无法提供 `plus.sqlite`，其初始化报错不作为 Android SQLite 结果。
- HBuilderX 真机待验证：日期 / 餐次紧凑行、四种支付、组合支付预填 1 次与步进上限、预占 / 不足文案、默认 / 无默认单价、pending 合并 / delivered 阻断 / 改单价确认、保存后二次弹窗及继续后的安全重置、固定确认区与 Android 返回键。

---

## v1.15（2026-07-23）

新建订单表单统一为单行字段布局与固定标签列；不改订单、金额、SQLite 或统计口径。

### UI 与工作流

- **一行一个字段**：日期、餐次及后续各业务字段均由独立 `uni-forms-item` 承载；移除独立“配送安排”色块，日期和餐次使用与其他字段相同的白色表面和分隔方式。
- **统一对齐**：所有订单表单字段使用固定 80px 标签列；标签与右侧表单控件垂直居中，控件左边界一致。实际单价提示移至控件列下方，不再影响该字段的垂直对齐。
- **组合支付一致性**：组合支付展开区取消额外横向缩进和灰色背景，与主表单使用同一白色表面；次卡次数与补款方式沿用相同标签列和控件起点，支付选项文字保持单行。
- **客户状态操作**：重新检查、查看已有订单和查看已配送订单使用 200rpx 最小宽度、64rpx 固定高度与两侧留白，操作文案保持单行，扩大触摸区并避免文字拥挤。
- **支付模式切换**：“改为纯支付”使用 200rpx 最小宽度、64rpx 固定高度和两侧留白，在 390px 窄屏下仍与组合支付摘要完整并列。
- **默认日期**：订单列表进入新建页时不再传递当前筛选日期，新建表单每次都以 `tomorrow()` 默认到第二天；用户仍可在表单内选择其他日期。
- **详情编辑同步**：订单详情编辑态同步当前新建页的连续白色表面、80px 标签列、单行字段、统一分隔线、辅助按钮触摸尺寸、实际单价提示和底部确认区。详情页既有微信 / 现金 / 次卡 / 组合支付四种选项使用两列布局，避免窄屏文字拥挤。

### 保持不变

- 一餐一单、组合支付入口和默认次卡 1 次、pending 预占、配送扣次、金额计算、校验和保存后的二次弹窗均不变。
- 详情编辑的总份数语义、目标订单合并确认、配送、取消、复制和删除行为均不变。
- 未新增依赖、schema、迁移或 API。

### 验证门禁

- `pnpm test`（22 条）、`pnpm type-check`、`pnpm lint`、`pnpm build:h5` 和 `git diff --check` 通过；H5 构建仅输出既有 uni-ui / Dart Sass 弃用警告。
- 390 × 844 H5 浏览器检查确认默认录单的统一白色表面，以及组合支付展开区背景、标签 / 控件对齐和“改为纯支付”按钮宽度；H5 本身无法提供 `plus.sqlite`，其初始化报错不作为 Android SQLite 结果。
- HBuilderX 真机待验证：日期选择、餐次与支付触摸、次卡次数步进器、组合支付展开区、备注输入及固定确认区的 Android 尺寸与触摸表现。
- HBuilderX 真机待验证：订单详情进入 / 退出编辑、四种支付选项、组合支付面板、客户提示操作、固定确认区和只读态操作区的 Android 尺寸与触摸表现。

---

## v1.16（2026-07-24）

修复 UTC 时间戳与设备本地日期混用，导致凌晨开次卡收入未计入当日利润的问题。

### 修复

- **首页次卡收入**：`getRangeSummary()` 将 `meal_cards.created_at` 先转为设备本地日期再筛选，本地时间 00:00–07:59 的开卡金额不再被误归到前一天。
- **统计日趋势**：`getDailyTrend()` 使用同一本地日期表达式过滤和分组，保证首页、今日 / 本周 / 本月 / 自定义对账口径一致。
- 保留 UTC ISO `created_at` 写入和已有数据；不改 schema、迁移、备份格式、订单或支出口径。历史记录在升级后会直接按正确本地日期重新聚合。

### 回归保护

- 新增 `tests/stats-timezone.test.cjs`，以 `Asia/Shanghai` 下 `2026-07-24 00:39` 对应的 UTC 时间戳验证首页收入 / 利润与日趋势均计入当地 7 月 24 日。
- 修复前两条用例稳定失败（实际 0，期望 460）；修复后通过。

### 验证门禁

- `pnpm test`（24 条）、`pnpm type-check`、`pnpm lint`、`pnpm build:h5` 通过；H5 仅输出既有 uni-ui / Dart Sass 弃用警告。
- HBuilderX 真机待验证：在 Android 设备本地时区下新开次卡后，首页和统计页当日收入 / 利润应立即增加对应金额。

---

## v1.17（2026-07-24）

次卡充值记录增加安全删除，用于撤销误开且从未扣次的记录。

### 行为与回滚策略

- **删除边界**：仅 `used_meals = 0` 的记录可发起删除；API 额外检查 `meal_card_usages` 和 delivered 订单引用，有任一历史依赖即拒绝，不重写已配送订单。
- **pending 预占**：删除后的客户余额池必须仍能覆盖全部待配送订单预占；不足时返回剩余 / 预占次数并整笔回滚，不自动修改支付。
- **引用处理**：引用待删卡的 pending 订单改绑到客户最早且仍有余额的次卡；cancelled 订单清空 `meal_card_id`，订单次数、金额、支付方式和状态不变。
- **原子性**：依赖检查、预占校验、订单改绑 / 清空和次卡硬删除全部位于同一 `db.tx()` 事务，任一步失败即恢复删除前状态。删除成功后，该笔开卡收入和可用次数一并移除。

### UI 与反馈

- 充值记录卡片底部增加独立危险操作区：未扣次记录可删除，已扣次记录保留“已扣次，不能删除”禁用态；按钮与点击卡片编辑分区。
- 确认弹窗明确展示充值日期、开卡金额和减少次数；预占冲突展示具体剩余 / 已预占次数与处理建议。执行期间锁定页面其他删除和编辑入口，成功后重新加载列表。
- 未新增 schema、迁移、依赖或备份字段；新备份自然不再包含已删除记录。

### 回归保护

- 新增 `tests/meal-card-delete.test.cjs`，覆盖成功删除与 FIFO 改绑、pending 预占不足的事务回滚、已扣次拒绝、usage 历史与 delivered 引用保护。

### 验证门禁

- `pnpm test`（30 条）、`pnpm type-check`、`pnpm lint`、`pnpm build:h5` 和 `git diff --check` 通过；H5 仅输出既有 uni-ui / Dart Sass 弃用警告。
- HBuilderX 真机待验证：删除按钮尺寸 / 禁用 / 触摸分区、确认与错误弹窗，以及 Android SQLite 下成功删除、预占拒绝和订单引用改绑。

---

## v1.18（2026-07-28）

新增每日菜单与社群文案模板，用于提前维护一周午 / 晚餐并直接复制群文案。

### 每日菜单

- 一天一条菜单，午餐、晚餐至少填写一项并支持多行；今天及未来与历史菜单分别排序展示。
- 新建默认当天，支持“保存并新增下一天”；日期冲突不覆盖或合并，而是进入已有菜单。
- 当前 / 历史列表和已保存编辑页均可套用当前默认模板复制；菜单硬删除且不保留编辑版本。
- 今日页增加“社群菜单”快捷入口，我的页增加菜单管理与文案模板入口，保留原四个底部 Tab。

### 文案模板与版本历史

- schema 升至 v6，新增 `daily_menus`、`message_templates`、`template_versions`；菜单日期、模板名称唯一，部分唯一索引保证至多一个默认模板。
- 模板支持日期、午餐、晚餐和显式条件区块；缺餐时删除完整餐次段落，保留固定说明并整理多余空行。
- 模板编辑页提供区块插入、即时语法错误与示例菜品预览；预览使用带主色边线的社群公告纸条，其余页面沿用既有 `$hej-*` 设计基线。
- 名称或正文实际修改前保存完整快照；历史无限保留，恢复前先快照当前内容且不改变默认状态。模板和历史一起硬删除。
- 内置“日常午晚餐”默认模板不包含 `@所有人`；只在 v6 迁移、旧备份升级或危险清空时生成，用户删除最后一个模板后普通启动不会偷偷恢复。

### 备份与兼容

- schema v6 全量备份新增菜单、模板和版本数组；v1-v5 旧备份继续支持导入并补内置模板。
- v6 中用户主动删除全部模板的空状态可原样备份恢复；危险清空会删除三张新表数据并恢复内置模板和 5 个默认支出分类。

### 回归保护与验证

- `tests/menu-template.test.cjs` 覆盖双餐 / 缺餐渲染、多行文本、`$&` / `$1` 字符保真、带空格标记、未知 / 未闭合 / 嵌套 / 错位标记。
- `tests/schema-v6.test.cjs` 保留 v5 订单回归，并验证 fresh schema v6 的日期唯一、模板名称唯一、单默认约束和迁移追加。
- `pnpm test`（39 条）、`pnpm type-check`、`pnpm lint`、`pnpm build:h5` 和 `git diff --check` 通过；H5 仅输出既有 uni-ui / Dart Sass 弃用警告。
- 390 × 844 H5 检查确认模板长正文滚动、插入按钮、社群文案预览和固定保存区可读；H5 的 `plus.sqlite` 初始化失败不作为 Android 结果。
- HBuilderX 真机待验证：v5 → v6 迁移、菜单 / 模板 / 版本 CRUD、默认模板接替、连续录入、复制剪贴板、v1-v6 备份往返、危险清空，以及表单触摸 / 滚动 / 视觉。

---

## v1.19（2026-08-09）

统一页面栈内的“返回现场”：从列表、详情或表单进入下级页面后，返回时保留业务交互状态并恢复原滚动上下文。

### 页面行为

- 所有现有 `navigateTo` 父页面统一通过页面实例级快照打开下级页面；搜索词、日期 / 筛选、当前分栏、订单折叠和未提交表单草稿继续由原页面实例持有。
- 父页面返回后自行刷新需要更新的本地业务数据；刷新期间使用遮罩阻止操作旧数据，完成渲染后再恢复滚动。刷新回调显式报告成败，部分 Store 成功但整体失败时回滚刷新前视图，并保留原现场、快照和既有错误提示。
- 条目顺序未变化时恢复原像素；条目移动时保持原条目在相同屏幕区域，删除时依次定位下一相邻项、上一相邻项，列表清空时回到顶部。
- 状态只在当前页面栈内存活；页面卸载、`switchTab`、`reLaunch` 和冷启动不持久化。键盘、弹窗、选择器、拖拽过程和执行中状态不会恢复。

### 实现与兼容

- 新增共享 composable，同时适配内部 `scroll-view` 和原生页面滚动；真实 offset 通过选择器查询，`scroll-top` 同值恢复使用 1px 受控脉冲保证 App 端触发。
- 新增纯定位解析器，把条目变化的优先级集中为“原像素 / 原条目 / 下一项 / 上一项 / 顶部”，不引入 Pinia 全局缓存、本地存储、SQLite、schema 或第三方依赖。
- 订单列表继续复用既有 `listScrollTop`、动态 `scroll-y` 和边缘自动滚屏；订单折叠改为显式受控状态。客户列表保留字母索引，返回恢复前先清除高优先级 `scroll-into-view`。
- 表单页保留草稿字段，只重新读取已有订单、目标订单和次卡可用量等辅助数据；辅助刷新失败时恢复离开前上下文，不覆盖未提交字段。返回弹窗、键盘和半截操作均不保留。
- 代码审查修复订单相邻项顺序：快照改用午餐 → 晚餐的实际渲染顺序，避免删除晚餐分组末项后跳到页面上方的午餐项；客户刷新失败时不再把既有“次”标记改成“普”。

### 回归保护与验证

- 新增 7 条纯定位回归，覆盖列表不变、原条目移动、删除后的下一 / 上一相邻项、无存活条目、空列表和无可测锚点。
- 新增测试 7/7、`pnpm type-check`、`pnpm lint`、`pnpm build:h5` 与 `git diff --check` 通过；H5 构建仅输出既有 uni-ui / Dart Sass 弃用警告。
- 代码审查修复后再次执行：定位测试 7/7、`pnpm type-check`、`pnpm lint` 与 `pnpm build:h5` 通过；H5 仍仅输出既有弃用警告。
- 全量 `pnpm test` 为 45/46：既有 `schema-v6.test.cjs` 在 Windows 下收到 sqlite3 的 CRLF 输出，但断言固定为 LF；该失败与本次返回现场代码无关，本次未改无关测试。
- HBuilderX Android 真机待验证：客户字母索引后进入详情再返回、订单拖拽后进入详情再返回、编辑后排序移动、删除后相邻定位、原生页面滚动、多级返回、系统返回键、表单草稿和刷新失败。

---

## v1.20（2026-08-12）

修订页面返回现场和今日午餐折叠行为，解决订单配送后排序变化把滚动条带到目标订单位置，以及午餐全部配送后未自动收起的问题。

### 页面返回现场

- 返回快照只保存离开前滚动像素和页面实例状态，不再记录业务数据条目、条目顺序、条目偏移或相邻条目。
- 所有列表和表单页面移除 `anchorKey` / `getItemKeys` / 条目定位入口；数据新增、编辑、配送、删除或排序变化后仍恢复原滚动像素，返回后无内容时回到顶部。
- 客户列表的字母索引仍可主动跳转，但临时索引目标在返回恢复前清除，不覆盖像素恢复。

### 午餐折叠

- 今天午餐非空且所有订单状态都是 `delivered` 时，首次加载或从未完成跃迁到完成时自动收起午餐面板；`cancelled` 不视为已送达。
- 用户手动展开 / 收起后保留手动状态；如果面板是系统自动收起的，新增待配送午餐订单后自动重新展开。
- 该规则只改页面展示状态，不改变订单状态流、排序、数据库或业务 API。

### 回归保护

- 返回定位回归覆盖内容顺序变化、条目删除、空列表、负数像素和内容仍可见时的像素恢复。
- 订单规则回归覆盖今日午餐完成条件、取消单边界、自动收起 / 重新展开和手动状态优先级。
- `node --test tests/*.test.cjs` 47/47、直接 `vue-tsc --noEmit`、ESLint、H5 build 与 `git diff --check` 通过；项目 `pnpm` 入口因当前 Node 20.19.5 不满足 pnpm 11.20.0 的 Node 22.13 要求而未执行成功。
- HBuilderX Android 真机待验证：订单配送返回后的像素位置、午餐自动折叠 / 手动展开、新增午餐订单重新展开，以及其他列表返回后的像素恢复。

---

## v1.21（2026-08-12）

新增订单详情的月卡信息复制与独立月卡文案模板维护，服务“配送后立即把最新次卡余额同步给客户”的操作闭环。

### 月卡文案模板

- schema 升至 v7，新增 `meal_card_message_templates` 与 `meal_card_template_versions`；数据、默认状态、历史版本和删除接替均与菜单文案模板独立。
- “我的”增加月卡文案模板入口，支持新建、编辑、设为默认、历史版本恢复和硬删除。
- 内置正文为用户给定的 `478元/20份` 文案；套餐描述直接写在正文中，可随意修改，不提供套餐变量。
- 模板只允许并要求 `{{本次使用份数}}`、`{{当前可用份数}}` 两个占位符；未知、缺失或未闭合占位符不能保存。
- 默认模板只在 v7 迁移、旧备份升级或危险清空时补入；普通启动不会恢复用户主动删除的最后一个月卡模板。

### 订单详情交互

- 已配送且 `meal_card_quantity > 0` 的订单显示“复制月卡信息”；复制直接写入系统剪贴板，不增加预览步骤。
- `{{本次使用份数}}` 使用当前订单的次卡份数，`{{当前可用份数}}` 使用配送后客户 active 次卡的实际剩余次数，不扣除其他 pending 订单预占；pending 预占只影响订单新建 / 编辑时的可用性校验。
- 标记已配送成功后只更新当前订单详情状态，不再 `navigateBack()`；按钮立即变为可复制月卡信息的状态。
- 没有默认月卡模板时保留复制入口，点击提示并可跳转维护页。

### 备份与验证

- v7 备份新增月卡模板与版本数组；v1-v6 旧备份导入时补内置月卡模板，v7 空数组状态可原样恢复。
- 新增月卡模板纯函数测试，schema / backup 回归同步覆盖 v7；Windows sqlite CLI 测试统一规范 CRLF 输出后再比较，避免环境换行差异造成误报。
- HBuilderX Android 真机待验证：v5 → v7 迁移、模板 CRUD / 历史恢复、配送后详情留存、剪贴板内容、无默认模板引导及窄屏触摸布局。
