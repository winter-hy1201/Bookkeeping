...
## 当前增量（Issue #14：[UI 重构 13/14] 备份恢复与危险清空）
...
| 14.3 | HBuilderX Android 模拟器（`emulator-5554` / Xiaomi 23046PNC9C）：当前工作区编译安装、SQLite 初始化、真实导出与文件存在检查、已保存备份只暂存、Android 本地文件选择器、导入覆盖确认、滚动可达和危险区视觉已验证；实际覆盖恢复与三次清空需在破坏性动作发生前确认。 | ⏳ 已完成非破坏验收；破坏性闭环待确认 |
