...
- 2026-06-15：拖拽滚动冲突 hotfix（v1.6）：订单列表长按拖拽把手与 scroll-view 滚动冲突修复。`src/pages/order/index.vue` 触摸事件挪到 drag-handle、阈值 10px 触发取代 longpress、激活后 preventDefault + stopPropagation 双保险；`memory-bank/CHANGELOG.md` 新增 v1.6 节；`memory-bank/architecture.md` 更新 src/pages/order/index.vue 描述与 §更新日志；`AGENTS.md §11` 新增 scroll-view + touchmove 互斥踩坑记录。
