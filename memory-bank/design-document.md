...
> 当前业务模型与流程基线；实现事实以 `src/`、`package.json` 和现有测试为准。最近更新：2026-09-01 文档职责收口。
...
[Tab 2] 订单
   ...
   ├─ 空态可直接进入新建订单（日期固定默认明天，不继承列表筛选日期）
   ├─ [+] 新建订单 → 按“对象 → 份数与支付 → 金额 → 备注”分区的录入表单
   └─ 点击订单 → 订单详情（编辑 / 标记已配送 / 取消）

[Tab 3] 统计
   ├─ 时间筛选（今日 / 本周 / 本月 / 自定义）
   ├─ 关键数字（入账收入 / 支出 / 利润 / 有效订单（单 / 份） / 平均每单收入）
...
- 模板语法与渲染实现以 `src/utils/menu-template.ts` 为准，纯函数边界由 `tests/menu-template.test.cjs` 覆盖；页面交互以现有菜单页面与契约测试为准。
...
- 月卡复制只对已配送且 `meal_card_quantity > 0` 的订单开放；月卡文案中的当前剩余份数取配送后客户 active 次卡的实际剩余次数，不扣除其他 pending 订单预占。新建 / 编辑订单的可用性校验仍按实际剩余减其他 pending 预占计算。
...
配送成功后详情页保留在当前页面，并用事务返回的最新订单刷新状态。若订单实际使用次卡，详情页显示“复制月卡信息”；复制时读取默认月卡模板，将 `{{本次使用份数}}` 替换为 `meal_card_quantity`，将 `{{当前可用份数}}` 替换为配送后客户 active 次卡的实际剩余次数，不扣除待配送订单预占。没有默认月卡模板时提示先维护模板并提供跳转入口。
...
- 该规则由 `AGENTS.md §5` 强约束；具体精度自测与背景见 `memory-bank/CHANGELOG.md v1.5`。
...
- 新建页以高频录单为主：日期和餐次直接放在连续录单卡的白色表面内，二者各占一条普通表单行，不再使用独立“配送安排”色块；所有字段共享固定标签列，标签与控件垂直居中、右侧控件左边界一致。客户、份数、支付、金额、备注按决策顺序排列。原有 `<uni-forms>` 校验、合并确认和次卡可用次数校验不变。
- 新建页每次打开都用 `tomorrow()` 初始化配送日期；订单列表跳转不传 `date`，页面也不读取列表日期参数。用户仍可在表单内手动修改日期。
- 客户上下文提示中的“重新检查 / 查看订单”操作保留明确的触摸高度和水平留白，文案保持单行，不随提示内容收缩成紧贴文字的按钮。
...
当前产品只面向 Android `app-plus` 单用户本地记账；SQLite 数据、备份 JSON 和现有页面闭环均不依赖后端服务。

- 当前不包含账号体系、云同步、在线支付、配送路径规划、库存追踪、复杂图表、CSV / PDF 导出、自定义支出分类和独立“关于”页。
- 危险清空属于备份恢复页内的三级风险操作；未来候选或范围调整只记录在 `memory-bank/CHANGELOG.md`，不在本文件维护第二份列表。
...
|---|---|
| 领域词汇 | `CONTEXT.md` |
| 当前业务模型、schema、状态机和流程 | 本文件与 `src/db/schema.ts` / `src/db/migrations.ts`；冲突时以代码为准 |
| 当前 UI 规则 | `docs/design.md`、`src/uni.scss` 与目标页面现场 |
| 文件职责与架构边界 | `memory-bank/architecture.md` 与实际目录 / 配置 |
| 功能变更、验证、阻塞和历史 | `memory-bank/CHANGELOG.md` |
