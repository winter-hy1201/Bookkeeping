...
- 2026-09-01：新增根目录 `README.md`，以当前实现说明盒记的业务定位、功能闭环、运行方式、Android SQLite 边界和主要文档入口。

- 2026-08-31：[UI 重构 14/14] 23 页集成验收与文档收口（Issue #15）——在 HBuilderX Android 模拟器（`emulator-5554`，1440×2560 @ 480dpi）对 23 个现有路由逐一实际进入并按 375×812 参考图核对结构、层级、密度、色彩、导航与主动作；真实客户 / 订单 / 次卡 / 支出 / 菜单 / 模板数据全部来自 SQLite。业务回归覆盖订单拖拽持久化、页面像素返回现场、组合支付、次卡预占与配送扣次、统计口径、模板历史恢复、系统返回与手势返回；备份破坏性闭环（导出 → 三次确认清空 → 导入恢复 → 重启核对）在模拟器完成并补齐 Issue #14 待确认项。应用代码零变更；Node 113/113、类型检查、ESLint、H5 构建、diff 检查通过；物理真机回归与人为错误态注入待执行。
