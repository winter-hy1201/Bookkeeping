...
**当前状态（2026-07-22）**：**v1.0 已发布**，后续 v1.1-v1.12 增量已落地；v1.12 增加一餐一单、次卡预占与“次卡 + 微信/现金”组合支付，schema 升至 v5，HBuilderX 真机回归待执行。
Phase 1-9 全部完成；9.3 真机性能压测 / 9.4 Release APK 按用户决策跳过，用 HBuilderX 标准基座 debug APK 直接侧载。
13 个页面、6 张表、4 个 Pinia store、3 个通用组件、5+ SQLite 已真机验证；Phase 8 6 条 E2E 流程全部通过；`bookkeeping-v1.db` 阶段基线已落地。
**下一步**：先完成 v1.12 的 HBuilderX 真机回归与 v4 → v5 数据库升级验证，再继续收集真实使用反馈。
...
4. `memory-bank/CHANGELOG.md` — **v1.0-v1.12 已实现功能 + 行为决策 + 已知限制**（看一眼就知道哪些不要重做；金额精度问题在 v1.5 修复，所有金额运算走 `src/utils/format.ts` 的 5 个 helper）
...
5. `.hbuilderx/launch.json` 的 `customPlaygroundType` 用 `local`（2026-06-16 起，v1.6 同时调整）—— 表示走「本地标准基座」而非「连接的真机」；若需切到具体设备调试，HBuilderX 顶部「运行」下拉里手动选设备即可，**不要**把 launch.json 改回 `device`（基座包名 `io.dcloud.HBuilder` 不变，db 路径仍按 §3 包名指纹走）。
...
| 8.3 | 次卡次数不够 | 当前可用 1 次 + qty=2 纯次卡 → 创建时拒绝；改为次卡 1 次 + 微信/现金 1 份后可保存 |
...
| 11.1 | 组合支付 | 2 份、次卡 1 次 + 微信 ¥25 → amount=25，配送只扣 1 次，删除只回滚 1 次 |
| 11.2 | 一餐一单 | 同客户 / 日期 / 餐次重复新增 → 保留原 ID / 排序、累加份数、备注去重合并；delivered 阻止新增 |
| 11.3 | pending 预占 | 实际剩 2 次、其他 pending 已占 1 次 → 新单最多再用 1 次；充值记录不能调低到预占以下 |
| 11.4 | v5 兼容 | v4 DB 自动回填旧次卡单次数；v1-v5 备份往返后支付拆分、usage 与统计一致 |
...
3. **同一客户 + 日期 + 餐次 = 1 张有效订单**。重复新增填写本次增量并合入已有 pending 订单，保留原 ID / `sort_order`；delivered 阻止继续新增，cancelled 不复活。
4. **不收配送费**（D4）。schema 无 `delivery_fee`。
5. **次卡按"次"无有效期**。`meal_cards` **无** `end_date` 字段、**无** `expired` 状态。**禁止**添加。
6. **允许次卡 + 一种货币渠道组合支付**。`meal_card_quantity` 记录次卡次数；有货币部分时 `payment_method` 记录唯一的 `wechat` / `cash` 渠道。禁止微信 + 现金或三种混用。创建 / 编辑必须按“实际剩余 - 其他 pending 预占”校验，不自动尽量用次卡，也不自动改支付。
7. **`orders.amount` 只记录货币部分**：`unit_price × (quantity - meal_card_quantity)`；纯次卡订单为 0。配送只扣 `meal_card_quantity`，不足时整笔事务回滚并引导进入编辑，不自动转换支付。
...
# 期望：6 张业务表 / 5 个分类 / user_version = 5
...
| ❌ 禁止 | 理由 |
|---|---|
| 引入 ORM（Prisma / TypeORM） | 6 张表直接写 SQL 成本更低；与 uni-app 编译兼容差 |
...
| 次卡配送报 `InsufficientCardError` 没处理 | UI 层没捕获 | `pages/order/detail.vue` 已捕获并提示“去编辑支付”；**不要**自动改单或在 store 层静默吞 |
...
| `<scroll-view scroll-y>` 子节点拖拽手势与外层滚动冲突,拖不到指定位置 / 页面抖动 | scroll-view 在 touchstart 阶段已进入滚动跟踪态,后续子节点 touchmove 的 `event.preventDefault()` 在 Android HBuilderX 标准基座下**不生效**(JS 层 preventDefault 被忽略);长按 500ms 触发后才处理已经太晚。**所有依赖 JS 层 preventDefault / `.stop.prevent` 修饰符 / `catchtouchmove` 的方案在 app-vue 标准基座都无效** | **不要对抗它,绕开它**:拖拽激活时把 `<scroll-view :scroll-y="listScrollable">` 置 `false`,直接摘掉滚动能力 → 必然生效(它摘除的是滚动能力而非拦截事件);`@scroll` 回写真实 `scrollTop` 规避 `:scroll-top` 受控「设一次同值不再滚」陷阱;长列表用 `setTimeout`(16ms)驱动 `:scroll-top` 做边缘自动滚屏,并反向修正 `dragState.startY` 让 targetIndex 跟随。参考 v1.6(2026-06-16)`pages/order/index.vue` |
| `ReferenceError: requestAnimationFrame is not defined`(App.vue onError)| **uni-app app-plus 逻辑层(`<script setup>`)跑在独立 JSCore/V8,无 DOM API**: `requestAnimationFrame` / `cancelAnimationFrame` / `window` / `document` 全不存在 | 改用 `setTimeout(fn, 16)` / `clearTimeout(id)`(16ms ≈ 60fps);需要 DOM 动画时改走 renderjs / view 层 |
