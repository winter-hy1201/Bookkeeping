...
| 触发分支 | 读取路由 |
|---|---|
| 数据模型、字段、订单、次卡、统计、备份或业务流程 | 读 `memory-bank/design-document.md` 的相关章节；检查实际 schema / API / tests；需要版本或历史边界时再读 `memory-bank/CHANGELOG.md` 的对应版本。 |
...
| SQLite、`plus.sqlite`、原生桥或 Android 异常 | 先读 `src/db/index.ts`、`memory-bank/architecture.md` 的数据层登记与 `memory-bank/CHANGELOG.md` 的相关验证记录；原生结论只从对应 Android 验证取得。 |
| 新建、删除、移动文件，或改变文件职责、路由、依赖 | 读 `memory-bank/architecture.md` 和实际目录 / 配置；完成后登记职责变化。 |
| 计划、里程碑、回归状态或历史验收 | 读 `memory-bank/CHANGELOG.md` 的相关条目；历史记录不替代当前实现检查。 |

## 2. 最小路径：范围与改动边界

- 盒记是 Android 侧载、单用户、本地 SQLite 的盒饭档口记账 App，闭环为接单 → 配送 → 对账。
- 当前 schema、脚本、依赖和版本以实际源码、配置与 `package.json` 为准；功能变更、验证、未验证项和阻塞只以 `memory-bank/CHANGELOG.md` 的历史记录为准；不要把可变快照复制到本文件。
...
- 代码编写、静态检查和 H5 构建使用现有 CLI；CLI 只运行 `package.json` 实际声明的脚本。Android 原生 SQLite、触摸和视觉验收走 HBuilderX「运行到 Android App 基座」；插件、包名和路径以 `src/manifest.json` 与当前 HBuilderX 配置为准。
...
- 按分支更新权威文档：当前产品决策、schema 和业务流程写 `memory-bank/design-document.md`；当前领域词汇写 `CONTEXT.md`；当前 UI 规范写 `docs/design.md`；文件职责或架构边界变化时写 `memory-bank/architecture.md`；功能变更、验证、未验证项、阻塞和后续候选只写 `memory-bank/CHANGELOG.md`。不再维护独立 progress、实施计划或调试交接记录，也不把这些文档的可变状态复制回本文件。
- 完成里程碑时，把实际验证范围写入 `memory-bank/CHANGELOG.md`；不再在仓库内留存二进制 DB 基线副本。
